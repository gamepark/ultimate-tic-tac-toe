    ...
    <Letterbox css={letterBoxStyle} top={0}>
      {game.board.map((row, rowIndex) =>
        row.map((grid, columnIndex) => {
            const canPlay = !game.constraint || (game.constraint.x === columnIndex && game.constraint.y === rowIndex)
            return <Grid key={`${rowIndex}_${columnIndex}`} grid={grid}
                         css={[gridPosition(rowIndex, columnIndex), !canPlay && darken]}
                         onCellClick={(gridRow, gridColumn) => mark && canPlay && play({x: columnIndex, y: rowIndex, i: gridColumn, j: gridRow, mark})}/>
          }
        )
      )}
    </Letterbox>
